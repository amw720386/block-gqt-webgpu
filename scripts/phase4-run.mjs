...
if(mode==='phase4-bench'||mode==='profile')execFileSync(process.env.BLOCKGTQ_PYTHON||'python',['-m','tests.oracles.export_attention','--bench'],{cwd:root,stdio:'inherit'});
...
const measured=p=>p.startsWith('src/attention/')||p.startsWith('src/runtime/');
let phase3_source_unchanged=true;
for(const [p,h] of Object.entries(old.source_sha256))if(measured(p)&&hashes[p]!==h){
  phase3_source_unchanged=false;
  if(mode==='phase4-bench')throw new Error(`Phase 3 baseline changed: ${p}`);
}
const correctness_gates={};
if(mode==='phase4-bench'){
  const file='2026-09-15T09-25-12-173Z-phase4-optimized-test-cpu.json';
  ...
  for(const [p,h] of Object.entries(hashes))if(measured(p)&&gpu.source_sha256[p]!==h)throw new Error(`Untested implementation: ${p}`);
...
const file=mode==='optimized-test'?`results/phase4-${name}-${mode}.json`:`benchmarks/raw/${name}-phase4-${mode}.json`;
const report={schema_version:1,mode,created_utc:now,git_commit:execFileSync('git',['rev-parse','HEAD'],{cwd:root,encoding:'utf8'}).trim(),git_dirty:true,
  source_sha256:hashes,correctness_gates,phase3_source_unchanged,os:{platform:os.platform(),release:os.release(),version:os.version(),arch:os.arch(),cpu:os.cpus()[0]?.model},...result};
if(file.startsWith('results/'))await mkdir(resolve(root,'results'),{recursive:true});
