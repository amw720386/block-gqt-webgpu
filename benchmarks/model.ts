...
export async function run(){
  const query=new URLSearchParams(location.search),target=modelTarget(query.get('model')??'smollm2'),onlyContext=Number(query.get('context'))||undefined;
  const adapter=await navigator.gpu.requestAdapter();if(!adapter)throw new Error('No adapter');const device=await adapter.requestDevice({requiredLimits:{maxBufferSize:adapter.limits.maxBufferSize,maxStorageBufferBindingSize:adapter.limits.maxStorageBufferBindingSize}}),errors:string[]=[];
  device.addEventListener('uncapturederror',e=>errors.push(e.error.message));const environment=gpuMetadata(adapter,device),weights=await Weights.load(device,target.base),tokenizer=await Tokenizer.load(target.base,weights.adapter.chat,weights.adapter.vocabSize);
  ...
  async function execute(ids:Uint32Array,compressed:boolean,maxTokens=25){
    const calibration=compressed?await Calibration.load(device,weights.adapter,target.calibrationBase,weights.manifest.revision):undefined;let model:Decoder|undefined;
    try{
      ...
      if(compressed&&model.encodedRows!==model.length*weights.adapter.layers*weights.adapter.kvHeads)throw new Error('Historical re-encode count');
  ...
  try{
    ...
    for(const length of onlyContext?[onlyContext]:[256,512,1024,2048,4096]){
    ...
    for(const prompt of onlyContext?[]:['Explain why plants need sunlight.','Give two tips for keeping a desk organized.','Name one planet in our solar system.']){
    ...
    for(const compressed of onlyContext?[]:[false,true]){
      let calibration:Calibration|undefined,model:Decoder|undefined;device.pushErrorScope('out-of-memory');
      try{
        if(compressed)calibration=await Calibration.load(device,weights.adapter,target.calibrationBase,weights.manifest.revision);model=await Decoder.create(weights,8192,32,calibration);
