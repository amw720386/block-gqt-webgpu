...
import {modelIntegration} from "./model-integration.js";
import {gpuMetadata} from "./phase2/phase2-test.js";
import {modelTarget} from "../../src/model/targets.js";
export async function run(){
  const selected=new URLSearchParams(location.search).get('model'),target=modelTarget(selected??'smollm2'),base=target.base,adapter=await navigator.gpu.requestAdapter();if(!adapter)throw new Error("No adapter");
  const device=await adapter.requestDevice(selected?{requiredLimits:{maxBufferSize:adapter.limits.maxBufferSize,maxStorageBufferBindingSize:adapter.limits.maxStorageBufferBindingSize}}:undefined),errors:string[]=[];device.addEventListener('uncapturederror',e=>errors.push(e.error.message));
  const weights=await Weights.load(device,base),calibration=await Calibration.load(device,weights.adapter,target.calibrationBase,weights.manifest.revision),tokenizer=await Tokenizer.load(base,weights.adapter.chat,weights.adapter.vocabSize),cases=[];
  try{
    if(!selected)for(const compressed of [false,true]){
    ...
    let tokenizerTests:number|undefined;if(selected){const integration=await modelIntegration(base,weights,calibration,tokenizer);cases.push(...integration.cases);tokenizerTests=integration.tokenizerTests;}
    return {passed:errors.length===0,errors,cases,model:weights.manifest,environment:gpuMetadata(adapter,device),tokenizer_tests:tokenizerTests};
  }finally{calibration.destroy();weights.destroy();device.destroy();}
}
