...
struct P {rows:u32,start:u32,head:u32,tokens:u32,q_heads:u32,gqa:u32,head_dim:u32,pad:u32}
...
fn compressed_dot(@builtin(workgroup_id) group:vec3<u32>,@builtin(local_invocation_index) lane:u32){
  let token=group.x;let item=group.y*128u+lane;
  if(lane<64u){
  ...
  if(item<p.rows*p.gqa){
    let row=item/p.gqa;let head=p.head*p.gqa+item%p.gqa;let index=(row*p.q_heads+head)*p.tokens+token;
