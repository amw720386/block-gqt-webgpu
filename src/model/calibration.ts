...
export class Calibration {
  ...
  static async load(device:GPUDevice){
    const m=await (await fetch('/fixtures/model/calibration.json')).json(),payload=await (await fetch(`/fixtures/model/${m.binary}`)).arrayBuffer();
    const sha=Array.from(new Uint8Array(await crypto.subtle.digest('SHA-256',payload)),b=>b.toString(16).padStart(2,'0')).join('');
    if(sha!==m.sha256||m.heads.length!==90||m.model_revision!=="12fd25f77366fa6b3b4b768ec3050bf629380bac"||m.upstream_commit!=="3fe14e7d4b8a6c85402818f7e22052527ed42e93")throw new Error("Calibration mismatch");
